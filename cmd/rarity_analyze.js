...
const getHatchingInfo = async () => {
  try {
    const hatchingRes = await fetch("https://dexstats.info/api/hatching.php");
    return hatchingRes.json();
  } catch (error) {
...
const analyzeRarity = async () => {
  ...
  let insertPunkStmt = db.prepare(
    "INSERT INTO punks VALUES (?, ?, ?, ?, ?, ?, ?)"
  );
  let insertTraitTypeStmt = db.prepare(
    "INSERT INTO trait_types VALUES (?, ?, ?, ?)"
  );
  let insertTraitDetailTypeStmt = db.prepare(
    "INSERT INTO trait_detail_types VALUES (?, ?, ?, ?)"
  );
  let insertPuntTraitStmt = db.prepare(
    "INSERT INTO punk_traits VALUES (?, ?, ?, ?)"
  );

  let count1 = config.collection_id_from;

  const hatchingInfo = await getHatchingInfo();

  collectionData.forEach((element,idx) => {
    ...
    const currentEl = hatchingInfo.find(el=>{
     return +el.id === (idx + 1)
    })


    const hatched = currentEl && currentEl.ishatched === "true" ? 1 : 0
    ...
    element.attributes.forEach((attribute) => {
      ...
      if (
        ...
        attribute.value.toLowerCase() == "nothing" ||
      ...
      if (!traitTypeCount.hasOwnProperty(attribute.trait_type)) {
        ...
        if (
          !_.isEmpty(attribute.display_type) &&
          attribute.display_type.toLowerCase() == "date"
        ) {
          traitDataType = "date";
        }
        insertTraitTypeStmt.run(
          traitTypeId,
          _.startCase(attribute.trait_type),
          traitDataType,
          0
        );
        ...
        if (!ignoreTraits.includes(attribute.trait_type.toLowerCase())) {
          traitTypeCount[attribute.trait_type] =
            traitTypeCount[attribute.trait_type] + 1;
        } else {
          traitTypeCount[attribute.trait_type] = 0;
        }
      }
      ...
      // Trait detail type
      if (
        !traitDetailTypeCount.hasOwnProperty(
          attribute.trait_type + "|||" + attribute.value
        )
      ) {
        insertTraitDetailTypeStmt.run(
          traitDetailTypeId,
          traitTypeIdMap[attribute.trait_type],
          attribute.value,
          0
        );
        traitDetailTypeIdMap[attribute.trait_type + "|||" + attribute.value] =
          traitDetailTypeId;
  ...
  let updateTraitTypeStmt = db.prepare(
    "UPDATE trait_types SET punk_count = :punk_count WHERE id = :id"
  );
  ...
  let updateTraitDetailTypeStmt = db.prepare(
    "UPDATE trait_detail_types SET punk_count = :punk_count WHERE id = :id"
  );
  ...
  let insertPunkTraitContStmt = db.prepare(
    "INSERT INTO punk_trait_counts VALUES (?, ?)"
  );
  ...
  const punkScoreStmt = db.prepare(
    "SELECT rarity_sum FROM punk_scores WHERE punk_id = ?"
  );
  const punkRankStmt = db.prepare(
    "SELECT COUNT(id) as higherRank FROM punk_scores WHERE rarity_sum > ?"
  );
  let updatPunkRankStmt = db.prepare(
    "UPDATE punk_scores SET rarity_rank = :rarity_rank WHERE punk_id = :punk_id"
  );
