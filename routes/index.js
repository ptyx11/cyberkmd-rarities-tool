...
router.get('/', function(req, res, next) {
  ...
  const useUnhatched =  req.query.use_unhatched
  ...
  if (!_.isEmpty(search)) {
    ...
    totalPunkCountQuery = totalPunkCountQuery+' WHERE punks.id = :punk_id ';
    totalPunkCountQueryValue['punk_id'] = search;

    punksQuery = punksQuery+' WHERE punks.id = :punk_id ';
    punksQueryValue['punk_id'] = search;
  } else {
    totalPunkCount = totalPunkCount;
  }

  if(useUnhatched != null && useUnhatched.length){
    punksQuery = punksQuery+' AND punks.hatched = :hatched ';
    punksQueryValue['hatched'] = +useUnhatched === 1 ? 0 : 1
    totalPunkCountQuery = totalPunkCountQuery+' AND punks.hatched = :hatched ';
    totalPunkCountQueryValue['hatched'] = +useUnhatched === 1 ? 0 : 1
  }
